...
// added functionnalities by the showtime finance's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when STIME mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 stimeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake STIME tokens to TheBushV1
    ...
    // Withdraw STIME tokens from STAKING.
